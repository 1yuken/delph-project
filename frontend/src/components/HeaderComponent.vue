...
import { 
  Search, ChevronDown, MessageSquare, CreditCard, User, 
  Settings, LogOut, HelpCircle, Menu, X, Home, Users,
  Shield, BookOpen
      ...
      <div class="flex items-center gap-3">
        ...
        <div class="relative" ref="searchContainer">
          ...
          >
            ...
            <!-- Результаты поиска -->
            <div v-if="isDropdownVisible && filteredItems.length" class="mt-2 max-h-60 overflow-y-auto">
      ...
      <div class="px-6 py-4 space-y-6">
        ...
        <div class="space-y-1">
          <RouterLink 
            ...
            :class="isActiveRoute('/') ? 'bg-[#F0F7FF] text-[#0A65CC]' : 'text-[#656565] hover:bg-[#F9F9F9]'"
          >
            <Home class="w-5 h-5" />
          ...
          <RouterLink 
            ...
            to="/specialists" 
            class="flex items-center gap-3 px-4 py-3 rounded-lg transition-colors"
            :class="isActiveRoute('/specialists') ? 'bg-[#F0F7FF] text-[#0A65CC]' : 'text-[#656565] hover:bg-[#F9F9F9]'"
          >
            <Users class="w-5 h-5" />
        ...
        <div class="space-y-1">
          <h3 class="px-4 text-xs font-semibold text-[#656565] uppercase tracking-wider">Информация</h3>
          
          <RouterLink 
            @click="toggleMobileMenu" 
            to="/how-it-works" 
            class="flex items-center gap-3 px-4 py-3 rounded-lg transition-colors"
            :class="isActiveRoute('/how-it-works') ? 'bg-[#F0F7FF] text-[#0A65CC]' : 'text-[#656565] hover:bg-[#F9F9F9]'"
          >
          ...
          <RouterLink 
            ...
            to="/guarantees" 
            class="flex items-center gap-3 px-4 py-3 rounded-lg transition-colors"
            :class="isActiveRoute('/guarantees') ? 'bg-[#F0F7FF] text-[#0A65CC]' : 'text-[#656565] hover:bg-[#F9F9F9]'"
          >
          ...
          <RouterLink 
            ...
            to="/rules" 
            class="flex items-center gap-3 px-4 py-3 rounded-lg transition-colors"
            :class="isActiveRoute('/rules') ? 'bg-[#F0F7FF] text-[#0A65CC]' : 'text-[#656565] hover:bg-[#F9F9F9]'"
          >
        ...
        <div v-if="isUserAuthenticated" class="space-y-1">
          <h3 class="px-4 text-xs font-semibold text-[#656565] uppercase tracking-wider">Личный кабинет</h3>
          
          <RouterLink 
            @click="toggleMobileMenu" 
            to="/messages" 
            class="flex items-center gap-3 px-4 py-3 rounded-lg transition-colors"
            :class="isActiveRoute('/messages') ? 'bg-[#F0F7FF] text-[#0A65CC]' : 'text-[#656565] hover:bg-[#F9F9F9]'"
          >
          ...
          <RouterLink 
            ...
            to="/finances" 
            class="flex items-center gap-3 px-4 py-3 rounded-lg transition-colors"
            :class="isActiveRoute('/finances') ? 'bg-[#F0F7FF] text-[#0A65CC]' : 'text-[#656565] hover:bg-[#F9F9F9]'"
          >
          ...
          <RouterLink 
            ...
            to="/profile" 
            class="flex items-center gap-3 px-4 py-3 rounded-lg transition-colors"
            :class="isActiveRoute('/profile') ? 'bg-[#F0F7FF] text-[#0A65CC]' : 'text-[#656565] hover:bg-[#F9F9F9]'"
          >
            <User class="w-5 h-5" />
          ...
          <RouterLink 
            ...
            to="/settings" 
            class="flex items-center gap-3 px-4 py-3 rounded-lg transition-colors"
            :class="isActiveRoute('/settings') ? 'bg-[#F0F7FF] text-[#0A65CC]' : 'text-[#656565] hover:bg-[#F9F9F9]'"
          >
        ...
        <div v-else class="space-y-1">
          <h3 class="px-4 text-xs font-semibold text-[#656565] uppercase tracking-wider">Аккаунт</h3>
          
          <RouterLink 
            @click="toggleMobileMenu" 
            to="/login" 
            class="flex items-center gap-3 px-4 py-3 rounded-lg transition-colors"
            :class="isActiveRoute('/login') ? 'bg-[#F0F7FF] text-[#0A65CC]' : 'text-[#656565] hover:bg-[#F9F9F9]'"
          >
          ...
          <RouterLink 
            ...
            to="/register" 
            class="flex items-center gap-3 px-4 py-3 rounded-lg transition-colors"
            :class="isActiveRoute('/register') ? 'bg-[#F0F7FF] text-[#0A65CC]' : 'text-[#656565] hover:bg-[#F9F9F9]'"
          >
            <User class="w-5 h-5" />
          ...
          <div class="flex items-center gap-8">
            ...
            <nav class="flex items-center gap-6">
              ...
              <RouterLink 
                to="/specialists" 
                class="flex items-center gap-1.5 text-sm font-medium transition-colors"
                :class="isActiveRoute('/specialists') ? 'text-[#0A65CC]' : 'text-[#656565] hover:text-[#222222]'"
              >
                <Users class="w-4 h-4" />
          ...
          <div>
            ...
            <div v-if="isUserAuthenticated" class="flex items-center gap-6">
              ...
              <RouterLink 
                to="/messages" 
                class="flex items-center gap-1.5 text-sm font-medium transition-colors"
                :class="isActiveRoute('/messages') ? 'text-[#0A65CC]' : 'text-[#656565] hover:text-[#222222]'"
              >
              ...
              <RouterLink 
                to="/finances" 
                class="flex items-center gap-1.5 text-sm font-medium transition-colors"
                :class="isActiveRoute('/finances') ? 'text-[#0A65CC]' : 'text-[#656565] hover:text-[#222222]'"
              >
                ...
                >
                  <div class="relative">
                    <img 
                      ...
                      :class="isAvatarMenuOpen ? 'border-[#0A65CC]' : 'border-transparent hover:border-[#E5E9F2]'"
                      src="/avatar-full.jpg" 
                      alt="Аватар пользователя" 
                    />
                    <div class="absolute bottom-0 right-0 w-3 h-3 bg-green-500 rounded-full border-2 border-white"></div>
                  </div>
                  <ChevronDown class="w-4 h-4 text-[#656565]" :class="{ 'transform rotate-180 text-[#0A65CC]': isAvatarMenuOpen }" />
                </button>
