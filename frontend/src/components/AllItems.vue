...
const fetchItems = async () => {
  ...
  try {
    const { items: fetchedItems } = await itemsApi.getAll()
    ...
    result = result.filter((item) => {
      ...
      return itemCategories.some((link) => {
        ...
        switch (selectedCategory.value) {
          case 'design':
            return ['дизайн', 'ui', 'ux', 'figma', 'photoshop'].some((term) =>
          ...
          case 'development':
            return [
              ...
              'react',
              'javascript',
            ].some((term) => linkLower.includes(term))
          case 'marketing':
            return ['маркетинг', 'реклама', 'seo', 'smm'].some((term) => linkLower.includes(term))
          case 'writing':
            return ['копирайтинг', 'текст', 'статьи', 'контент'].some((term) =>
              linkLower.includes(term),
            )
          default:
            return false
  ...
  <section class="py-8 bg-white">
    <div class="w-3/5 mx-auto max-xl:w-4/5 max-lg:w-full max-lg:px-4">
      ...
      <div v-if="isLoading" class="flex justify-center py-8">
        <div class="animate-pulse flex flex-col items-center">
          <div class="rounded-full bg-[#F0F7FF] h-12 w-12 flex items-center justify-center mb-4">
            <svg
              class="w-6 h-6 text-[#0A65CC] animate-spin"
      ...
      <div v-else class="flex gap-11.5 max-md:gap-8">
        ...
        <aside
          class="text-[#656565] text-xs flex flex-col gap-2 max-w-[70px] w-full h-screen sticky top-[95px]"
        >
          <span
